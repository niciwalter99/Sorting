...
static uint8_T c1_e_emlrt_marshallIn
  (SFc1_SortierAlgorithmusVirtuelleTestumgebungInstanceStruct *chartInstance,
   const mxArray *c1_b_tp_firstEntry, const char_T *c1_identifier);
   ...
   const mxArray *c1_b_tp_firstEntry, const char_T *c1_identifier)
{
  ...
  c1_thisId.fParent = NULL;
  c1_thisId.bParentIsCell = false;
  c1_y = c1_f_emlrt_marshallIn(chartInstance, sf_mex_dup(c1_b_tp_firstEntry),
    &c1_thisId);
  sf_mex_destroy(&c1_b_tp_firstEntry);
  return c1_y;
}
...
static void c1_d_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c1_mxArrayInData, const char_T *c1_varName, void *c1_outData)
{
  const mxArray *c1_b_tp_firstEntry;
  const char_T *c1_identifier;
  emlrtMsgIdentifier c1_thisId;
  uint8_T c1_y;
  SFc1_SortierAlgorithmusVirtuelleTestumgebungInstanceStruct *chartInstance;
  chartInstance = (SFc1_SortierAlgorithmusVirtuelleTestumgebungInstanceStruct *)
    chartInstanceVoid;
  c1_b_tp_firstEntry = sf_mex_dup(c1_mxArrayInData);
  c1_identifier = c1_varName;
  c1_thisId.fIdentifier = (const char *)c1_identifier;
  c1_thisId.fParent = NULL;
  c1_thisId.bParentIsCell = false;
   ...
   uint32_T c1_b)
{
  ...
  switch (c1_b) {
   case 152U:
    sf_mex_assign(&c1_m0, c1_sfAfterOrElapsed(chartInstance), false);
   ...
   case 159U:
    sf_mex_assign(&c1_m3, c1_d_sfAfterOrElapsed(chartInstance), false);
  ...
  (void)
{
  ...
  {
    mxArray *mxChecksum = mxCreateString("7SpXNrq5GlgCFplJcsVX1G");
